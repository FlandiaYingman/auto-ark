...
import dev.flandia.ark.operate.OperateOperations.CA_5
import dev.flandia.ark.operate.OperateOperations.CE_5
import dev.flandia.ark.operate.OperateOperations.CE_6
import dev.flandia.ark.operate.OperateOperations.LS_5
import dev.flandia.ark.operate.OperateOperations.LS_6
import dev.flandia.ark.operate.OperateOperations.MAIN_1_7
import dev.flandia.ark.operate.OperatePoses.PR_X_1
import dev.flandia.ark.operate.OperatePoses.PR_X_2
import dev.flandia.ark.operate.OperatePoses.XX_5
import dev.flandia.ark.operate.OperatePoses.XX_5_OLD
import dev.flandia.ark.operate.OperatePoses.XX_6
import dev.flandia.ark.operate.OperatePoses.终端
import dev.flandia.ark.operate.OperatePoses.终端_主题曲
import dev.flandia.ark.operate.OperatePoses.终端_常态事务
import dev.flandia.ark.operate.OperatePoses.终端_活动
import dev.flandia.ark.operate.OperatePoses.终端_资源收集
import dev.flandia.ark.operate.OperateTemplates.剿灭_作战进度奖励_已满
import dev.flandia.ark.operate.OperateTemplates.剿灭_已刷满
import dev.flandia.ark.operate.OperationType.剿灭
import dev.flandia.ark.resetInterface
import dev.flandia.ark.tmpl
import dev.flandia.ark.today
...
object OperateTemplates {
    // 关卡准备页面，且代理指挥开启
    internal val 关卡信息界面_代理指挥开启 by tmpl()

    // 关卡准备页面，且代理指挥关闭
    internal val 关卡信息界面_代理指挥关闭 by tmpl()

    // 关卡准备页面，可以全权委托，并且全权委托为开启状态
    internal val 关卡信息界面_全权委托开启 by tmpl()

    // 关卡准备页面，可以全权委托，并且全权委托为关闭状态
    internal val 关卡信息界面_全权委托关闭 by tmpl()

    internal val 全权委托确定界面 by tmpl()

    // 等待“开始行动”
    internal val 编队界面 by tmpl()

    // 理智不足
    internal val 理智不足_可使用药剂 by tmpl()

    // 理智不足，且即将过期
    internal val 理智不足_药剂即将到期 by tmpl()

    // 理智不足，且仅能使用源石补充
    internal val 理智不足_可使用源石 by tmpl()

    // 等待“作战结束”
    internal val 行动结束 by tmpl()

    // 等待“升级”
    internal val 等级提升 by tmpl(diff = 0.025)

    internal val 剿灭_行动结束 by tmpl(diff = 0.05)
    internal val 剿灭_已通过 by tmpl()
